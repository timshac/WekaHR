...
int Modbus::poll() {
    ...
    if ( available_len > 0 ) {
        ...
        //delay(10);
        // old data is too old
        if ( micros( ) > ( last_receive_time + 2 * 1000 * 1000 ) || got_good_packet ) {  //   2 seconds
            lengthIn = 0;
        }
        got_good_packet = false;
        // we waited for the inter-frame timeout, read the frame.
        lengthIn += serial.readBytes( (char *) bufIn + lengthIn, 
                                      MAX_BUFFER - lengthIn );
    ...
    if (word(bufIn[lengthIn - 1], bufIn[lengthIn - 2]) == calcCRC(bufIn, lengthIn - 2) ) {  // if wire-crc == calc-crc
        ...
        Log.info("Serial: %s", data.c_str());
        
        // check unit-id
        Log.info("Unit: %d, fc: %d, address:%d, length:%d, wire-crc:%d, calc-crc:%d | lengthIn: %d", 
            bufIn[0],bufIn[1],word(bufIn[2], bufIn[3]), word(bufIn[4], bufIn[5]),
            word(bufIn[lengthIn - 1], bufIn[lengthIn - 2]), // wire-crc
            calcCRC(bufIn, lengthIn - 2), // calc-crc
            lengthIn);

        //Particle.publish("serial", String::format("fc: %d, address2: %d, length: %d",fc,address,length).c_str(), PRIVATE);

    } else {
        return 0;
    }

    //Log.info("Unit: %s\n",String(bufIn[0], HEX).c_str());

    if (bufIn[0] != unitID) return 0;
    
    //Particle.publish("serial", "unitID match", PRIVATE);
    
    //Particle.publish("serial", String::format("length: %d",lengthIn), PRIVATE);
    ...
    switch (fc) {
        ...
        case FC_WRITE_MULTIPLE_REGISTERS:
            ...
            //Particle.publish("serial", "FC_WRITE_MULTIPLE_REGISTERS", PRIVATE);
     ...
     */
    cb_status = STATUS_OK;
    //Particle.publish("serial", "parse command", PRIVATE);
